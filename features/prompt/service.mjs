...
export const PROMPT_TYPES = ['research', 'terminal', 'memory'] as const;

export interface PromptState {
    [key in typeof PROMPT_TYPES[number]]?: string;
}

let activePrompts: PromptState = {};
...
OPERATING PRINCIPLES:\n` +
    ...
    researchExpansion(query): string {
...
async function persistToGitHub(typeKey: string): Promise</void> {
...
export async function loadDefaultConfig(
    type: 'research' | 'terminal' | 'memory'
): Promise<Pick<PromptState, string>> {
    ...
    try {
        ...
        activePrompts[type] ??= JSON.parse(rawJson);
